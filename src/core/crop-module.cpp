...
void CropModule::step(double vw_MeanAirTemperature,
  ...
  // // FS: calculate height-dependent atm. pressure and psychrometric constant (calculating this once should be good enough)
  // if (_noOfCropSteps == 0) {
      ...
      for (int h = 0; h < 24; ++h) {  // hourly overclocked loop
        ...
        */
        ...
        if (cropPs.__enable_leaf_temperature__ && _noOfHourlySteps_Devstage_gt_0 > 0) {                                                 // FS: using a simple canopy temperature implementation
          ...
          assert(std::isfinite(globRad_Wpm2ps)); // FS DEBUG LAI
          ...
          double Tc_K = canopTemperature(globRad_Wpm2ps, Ta_K,
...
#pragma region apply photo reduction factors // apply factors that reduce (gross) photosynthesis
        ...
        //     In step(...), the fc_CropWaterUptake(...) method is called after fc_CropPhotosynthesis, so the transpiration deficit of the previous step is used?
...
tuple<double, double, double, double> CropModule::vc_KTkc_vc_KTko(double vw_MeanAirTemperature) const {
  ...
  /* FS: maybe also introduce option to choose WangEngelTemperatureResponse here for consistency? 
  double vc_KTkc = cropPs.__enable_Photosynthesis_WangEngelTemperatureResponse__
                   ? max(0.00001, WangEngelTemperatureResponse(vw_MeanAirTemperature,
                                                               pc_MinimumTemperatureForAssimilation,
                                                               pc_OptimumTemperatureForAssimilation,
                                                               pc_MaximumTemperatureForAssimilation,
                                                               1.0))
                   : exp(speciesPs.AEKC * term1) * term2;
  double vc_KTko = cropPs.__enable_Photosynthesis_WangEngelTemperatureResponse__
                   ? max(0.00001, WangEngelTemperatureResponse(vw_MeanAirTemperature,
                                                               pc_MinimumTemperatureForAssimilation,
                                                               pc_OptimumTemperatureForAssimilation,
                                                               pc_MaximumTemperatureForAssimilation,
                                                               1.0))
                   : exp(speciesPs.AEKO * term1) * term2;
  */
...
CropModule::A_rubisco_results CropModule::A_rubisco(double vw_MeanAirTemperature, double Cc, double O, Voc::CPData &_cropPhotosynthesisResults) const {
  ...
  // FS: remove the * vc_Vcmax / vc_Vcmax calculations?
  ...
  // FS: radiation use efficiency == 0.0 causes a zero-division problem in the hourly light response curve
  //     since that would be questionable anyways, maybe it is actually good if the model exits in a controlled way in that case
  assert(vc_RadiationUseEfficiency > 0.);
  assert (vc_RadiationUseEfficiencyReference > 0.);
...
void CropModule::fc_CropPhotosynthesis(double vw_MeanAirTemperature,
  ...
  double PHCH1 = SSLAE * vc_AssimilationRate * vc_EffectiveDayLength * X / (1.0 + X); // = HERMES // FS: X / (1.0 + X) [function type c*x/(x+c)] is a rectangular hyperbola light response curve
                                                                                                  //     other light response curve approaches often seem to use an exponential version 1.0-exp(-x)
                                                                                                  //     [function type c*(1.0-exp(-x/c))] instead (see photosynthesis-hourly.h/.cpp for more details);
                                                                                                  //     gas exchange measurements often seem to use rectangular hyperbola or non-rectangular hyperbola
                                                                                                  //     light responses e.g. of types x+c-sqrt((x+c)^2-4x*c*theta) or x+c-sqrt((x+c)^2-4x*c)/(2*theta)
                                                                                                  //     with x = RadiationUseEfficiency * PAR irradiance and c = maximum (saturation) AssimilationRate
...
#pragma endregion hourly FvCB code
  ...
  // daily GrossCO2Assimilation, multiplication with stress factors FieldCOnditionModifier, vc_CropFrostRedux, vc_OxygenDeficit, vc_TranspirationDeficit) and mainenance respiration
...
CropModule::GP_results CropModule::fc_CropGrossPhotosynthesis_h(double inst_diff_rad,
  ...
  if ((inst_diff_rad <= 0) && (inst_dir_rad <= 0)) {
    ...
    if (pc_CarboxylationPathway == 1) {
      ...
      // _cropPhotosynthesisResults.ci = Ci;
      assert(std::isfinite(Ci));

      double Oi = Oi_empirical(leafTemperature);
      // _cropPhotosynthesisResults.oi = Oi * 1000.0;  // mmol -> umol
      assert(std::isfinite(Oi));
    ...
    assert(std::isfinite(vc_AssimilationRate_hourly));
    assert(std::isfinite(vc_RadiationUseEfficiency_hourly));
    assert(vc_RadiationUseEfficiency_hourly > 0.0);
...
CropModule::crop_r CropModule::crop_aerodyn_stomata_surface_resistances(double vc_SaturatedVapourPressure,
  ...
  double vc_PsycrometerConstant = 0.000665 * vc_AtmosphericPressure;                             // psychrometric constant [kPa °C-1]

  // Calculation of wind speed in 2m height
  double vc_WindSpeed_2m = max(0.5, vw_WindSpeed * (4.87 / (log(67.8 * vw_WindSpeedHeight - 5.42))));   // 0.5 minimum allowed windspeed for Penman-Monteith-Method FAO style calculations

  // aerodynamic resistance (for the actual crop as opposed to FAO-56 12 cm grass)
  ...
  double crop_AerodynamicResistance = (log(arg) * log(arg / 0.1)) / ((0.41 * 0.41) * vc_WindSpeed_2m);  // [s m-1] FAO-56 eq.4 and box 4

  // stomata resistance (to H2O) of the actual crop (used as approx. stomata conductance to heat here)
  double vc_SaturationDeficit = vc_SaturatedVapourPressure - vc_VapourPressure;
  ...
  if (A_gross_mol < 0.1) {
    crop_StomataResistance = 2.;  // [s m-1]   // FS: check what leads to a realistic leaf energy balance here: maybe 10000? or 5000? or 2000 or ...?
    ...
    double crop_StomataResistance = 20. * (1e-6*vw_AtmosphericCO2Concentration * (1.0 + vc_SaturationDeficit / 1.5)) / (A_gross_mol);  // [s m-1]
    cerr << " ->calc: crop_StomataResistance=" << crop_StomataResistance << std::endl;    // FS DEBUG LAI
    assert(std::isfinite(crop_StomataResistance));
  }

  // surface resistance (to H2O) of the actual crop
  double crop_SurfaceResistance = crop_StomataResistance / LAI_active;                           // [s m-1] FAO-56 eq.5

  return {crop_AerodynamicResistance, crop_StomataResistance, crop_SurfaceResistance};
}
 ...
 * @param vc_VapourPressure               actual vapour pressure e_a [kPa]
 * @param vc_SaturatedVapourPressure      saturation vapour pressure e_s [kPa]
 * @param vc_SaturatedVapourPressureSlope S = d(e_s)/dT [kPa °C-1]
 * @param vc_PsycrometerConstant
 * @return double canopy temperature [K]
 */
double CropModule::canopTemperature(double globalRad_Wpm2ps,
                                    double Ta_K,
  ...
  double Ri = la_sw * globalRad_Wpm2ps + la_lw * StefanBoltzmanConstant * pow(Te_K, 4.);  // leaf absorption in short-wave and long-wave radiation, Yu et al. eq.9

  // assuming that crop_AerodynamicResistance for heat and for water vapor is similar
  double he = rho_air * Cp_air / (1e3*vc_PsycrometerConstant * (crop_AerodynamicResistance + crop_StomataResistance));  // water tranfer coefficient, Paw 1987 eq.14a (=Yu et al. 2001 eq.13)
  double ht = rho_air * Cp_air / crop_AerodynamicResistance;                                                        // heat transfer coefficient, Paw 1987 eq.14b (=Yu et al. 2001 eq.14)

  // // dry surface temperature (operative temperature)
  // // Paw 1987 eq.4 and eq.5
  // double term1 = epsi * StefanBoltzmanConstant * Ta_K * Ta_K;
  // double a = 6. * term1;
  // double b = ht + 4. * term1 * Ta_K;
  // double c = term1 * Ta_K * Ta_K - Ri;
  // double term2 = b * b - 4. * a * c;
  // assert(term2 > 0.);
  // double delta_T = (-b + sqrt(term2)) / (2. * a); // positive sqrt
  // double delta_T = (-b - sqrt(term2)) / (2. * a); // negative sqrt

  // // wet surface temperature of an organism
  // // linear Taylor approximation, Paw 1987 eq.14 (=Yu et al. 2001 eq.11)
  // double term1 = epsi * StefanBoltzmanConstant * Ta_K * Ta_K;
  // double num = Ri - term1 * Ta_K * Ta_K -  he * (vc_SaturatedVapourPressure - vc_VapourPressure);
  // double denom = term1 * Ta_K + ht + he * vc_SaturatedVapourPressureSlope;
  // // double num = (Ri - epsi * StefanBoltzmanConstant * pow(Ta_K, 4.) - he * (vc_SaturatedVapourPressure - vc_VapourPressure));
  // // double denom = (4 * epsi * StefanBoltzmanConstant * pow(Ta_K, 3.) + ht + he * S); // num/denom results in a temperture difference [°C or K, which is eqivalent for a difference]
  // double delta_T = (num / denom);

  // quadratic Taylor approximation, Paw 1987 eq.15
  ...
  double c = term1 * Ta_K * Ta_K - Ri + he * 1e3*vc_VapourPressure;
  double term2 = b * b - 4. * a * c;
  assert(term2 > 0.);
  double delta_T = (-b + sqrt(term2)) / (2. * a); // positive sqrt
  // double delta_T = (-2. * c) / (b + sqrt(term2)); // algebraic equivalent for positive sqrt; should be more numerically stable 
  // double delta_T = (-b - sqrt(term2)) / (2. * a); // negative sqrt; not physically meaningful

  return Ta_K + delta_T;                                                                                    // canopy temperature = air temperature + temperature difference
}
